...
This GitHub repo starts **private**. Cursor Marketplace listing requires a **public** open-source repo. After org permission, set visibility to public, then submit at https://cursor.com/marketplace/publish.
...
3. Sign in when prompted (Open Video OAuth in Cursor). This package has **no API key**. Auth is the hosted MCP OAuth flow, not a secret in `mcp.json`.
...
## Official listing (after the repo is public)

Search **Open Video** in Cursor Marketplace and in Grok Bot → Plugins (same catalog).
